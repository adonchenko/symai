...
class SymAITestCase(unittest.TestCase):
    def test_get_vars_using_assignment(self):
        test_data = [
                        [
                            "(a.d.f==3==b) && c(s) > d && e(125) == 22|| f == g + 125 && h <= i",
                            [{'a.d.f': 3.0, 'b': 3.0, 'e(125)': 22.0}, {'f': 'g+125'}, "c(s)>d||h<=i"]
                        ],
                        [
                            "(a.d.f!=3==b) && c(s) > d && e(125) == 22|| f == g + 125 && h <= i",
                            [{'b': 3.0, 'e(125)': 22.0}, {'f': 'g+125'}, "(a.d.f!=3)&&c(s)>d||h<=i"]
                        ]
                    ]
        for it in test_data:
            ...
            for i in a.keys():
                ...
                for j in it[1][0].keys():
                    if it[1][0][j] == a[i]:
                        f = True
                        break
                self.assertTrue(f, f"Variable {i} must not be included to extracted constants")
            ...
            for i in b.keys():
                ...
                for j in it[1][1].keys():
                    if it[1][1][j] == b[i]:
                        f = True
                        break
                msg = f"Variable {i} must not be included to extracted symbolic expressions"
                self.assertEqual(f, True, msg)
